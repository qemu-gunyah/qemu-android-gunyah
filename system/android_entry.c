...
/* Environment variable used to pass a custom resolv.conf path to SLIRP.
 * setenv/getenv are process-wide so no mutex is needed — Java calls
 * setResolvConfPath() before startQemu() on the same process. */
#define RESOLV_CONF_ENV "SLIRP_RESOLV_CONF"
...
Java_com_vectras_qemu_jni_Loader_setResolvConfPath(JNIEnv *env, jclass clazz, jstring path) {
    ...
    if (path) {
        ...
        if (str) {
            setenv(RESOLV_CONF_ENV, str, 1);
            __android_log_print(ANDROID_LOG_INFO, "QEMU-system",
                                "resolv.conf env set: %s=%s", RESOLV_CONF_ENV, str);
            (*env)->ReleaseStringUTFChars(env, path, str);
        }
    } else {
        unsetenv(RESOLV_CONF_ENV);
    }
}
